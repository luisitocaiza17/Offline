...
public class CotizadorAgricola extends JDialog {
    ...
    private Boolean ObservacionValor=false;
    ...
    public CotizadorAgricola(String objetoId, ConsultarCotizacion cc){
        ...
        //Ocultamos campo de observaciones para tener mas espacio.
        lblAnalisisObservaciones.setVisible(false);
        txtAnalisisObservaciones.setVisible(false);
        ...
        grabarButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (ObservacionValor)
                {
                    JOptionPane.showMessageDialog(null,"La suma asegurada ingresada no puede ser mayor a la suma asegurada sugerida");
                    //txtCedula.Focus();
                    txtAnalisisMontoAsegurado.requestFocus();
                    return;
                }
                ...
                if(regla!=null){
                    if(txtAnalisisMontoAsegurado.isEnabled()||txtAnalisisMontoAsegurado.isEditable())
                        ReCalcularValores(regla);
                    else
                    ...
                    txtAnalisisMontoAsegurado.setEnabled(false);
                    txtAnalisisMontoAsegurado.setEditable(false);
                    txtAnalisisMontoAsegurado.setText("0.0");
                    txtAnalisisMontoRecom.setText("0.0");
                }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (cmb_Canton.getSelectedIndex()!=0 && cmb_TipoCultivo.getSelectedIndex()!=0){
                    ...
                    if(regla!=null){
                        ...
                        txtAnalisisMontoAsegurado.setEnabled(false);
                        txtAnalisisMontoAsegurado.setEditable(false);
                        txtAnalisisMontoAsegurado.setBackground(Color.decode("#BBEBFF"));
                        txtAnalisisMontoAsegurado.setText("0.0");
                        txtAnalisisMontoRecom.setText("0.0");
                    }
                }
        ...
        txtAnalisisMontoAsegurado.addKeyListener(new KeyAdapter() {
            @Override
            public void keyReleased(KeyEvent e) {
                super.keyReleased(e);
                if (txtAnalisisMontoAsegurado.getText().isEmpty()){
                    setCamposValores();
                    return;
                }
        ...
        btnPendiente.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if ((tipoId.getId().toString().equals("1")|| tipoId.getId().toString().equals("2"))
                        && txtNumeroIdentificacion.getText().length()<10)
                {
                    JOptionPane.showMessageDialog(null,"Error validación de documento. debe ingresar el número de IDENTIFICACIÓN ");
                    return;
                }
                if ((tipoId.getId().toString().equals("3")|| tipoId.getId().toString().equals("4"))
                        && txtNumeroIdentificacion.getText().length()<13)
                ...
                if (!strResult.isEmpty())
                {
                    JOptionPane.showMessageDialog(null,"Error validación de documento. Identificación: "+ strResult);
                    //txtCedula.Focus();
                    txtNumeroIdentificacion.requestFocus();
                    return;
                }
        ...
        txt_Latitud.addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) {
                ...
                if (txt_Latitud.getText().length()==10 && !txt_Latitud.getText().contains(".")){
                    if (c!='.')
                        e.consume();
                }
        ...
        txt_longitud.addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) {
                ...
                if (txt_longitud.getText().length()==10 && !txt_longitud.getText().contains(".")){
                    if (c!='.')
                        e.consume();
                }
    ...
    public void GrabarCotizacion(Integer estado) throws ParseException {
        ...
        cotizacion.setDireccionSiembra(txtDireccionReferencia.getText().replace("Á", "|A").replace("É","|E").replace("Í","|I").replace("Ó","|O").replace("Ú", "|U")
                .replace("á", "|a").replace("é", "|e").replace("í","|i").replace("ó","|o").replace("ú","|u")
                .replace("Ñ", "-").replace("ñ", "_"));
        cotizacion.setNumeroIdentificacion(txtNumeroIdentificacion.getText());
        cotizacion.setNombres(txtNombres.getText().replace("Á", "|A").replace("É","|E").replace("Í","|I").replace("Ó","|O").replace("Ú", "|U")
                .replace("á", "|a").replace("é", "|e").replace("í","|i").replace("ó","|o").replace("ú","|u")
                .replace("Ñ", "-").replace("ñ", "_"));
        cotizacion.setApellidos(txtApellidos.getText().replace("Á", "|A").replace("É","|E").replace("Í","|I").replace("Ó","|O").replace("Ú", "|U")
                .replace("á", "|a").replace("é", "|e").replace("í","|i").replace("ó","|o").replace("ú","|u")
                .replace("Ñ", "-").replace("ñ", "_"));
    ...
    public  void CalcularValores(Regla regla){
        ...
        if(regla!=null) {
            if (cultivo.getNombre().contains("VARIABLE")) {
                txtAnalisisMontoAsegurado.setEnabled(true);
                txtAnalisisMontoAsegurado.setEditable(true);
                txtAnalisisMontoAsegurado.setBackground(Color.green);
            } else {
                txtAnalisisMontoAsegurado.setEnabled(false);
                txtAnalisisMontoAsegurado.setEditable(false);
                txtAnalisisMontoAsegurado.setBackground(Color.decode("#BBEBFF"));
        ...
        if (prima.compareTo(new BigDecimal("0"))>0) {
           ...
           //Hacemos la distincion por provincia para aplicar ley de solidaridad iva 14% exceptamos MANABI Y ESMERALDAS
            ...
            if(ProvinciaExcetion.toUpperCase().trim().equals("MANABI")||ProvinciaExcetion.toUpperCase().trim().equals("ESMERALDAS")){
                Iva = (new BigDecimal("12").multiply(prima.add(superBanc).add(DerecEmision).add(SeguroCam))).divide(divisor);
            }else {
                //Ingreso fecha de fin de ley de Solidaridad 01/06/2017 a partir de la cual el iva regresa al 12%
                Date fechaActual = new Date();
                SimpleDateFormat sm = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
                String fechaFinLey = "31/05/2017 23:59:00";
                Date fechaFin = new Date();
                try {
                    fechaFin = sm.parse(fechaFinLey);
        ...
        if(regla.getObservaciones()!=null) {
    ...
    /**PROCESO DE RECALCULO EN CASO DE QUE SEA UN CULTIVO CON MONTO ASEGURADO VARIABLE **/
    public  void ReCalcularValores(Regla regla) {
        ReglaId = regla.getReglaId();
        String recomendacion = "";
        String ObservacionRegla = "";
        Locale local = new Locale("es", "EC");
        ...
        /*tomamos el valor del monto asegurado para calcular el costo de produccion */
        BigDecimal nuevoMontoAsegura = new BigDecimal(txtAnalisisMontoAsegurado.getText());
        BigDecimal hectareasAsegurdas = new BigDecimal(txtTotalHecAsegurables.getText());
        BigDecimal nuevoCostoProduccion = nuevoMontoAsegura.divide(hectareasAsegurdas);

        BigDecimal CostoProduccion = nuevoCostoProduccion;
        BigDecimal TotalHec = new BigDecimal(txtTotalHecAsegurables.getText());
        BigDecimal banc = new BigDecimal("3.50");
        BigDecimal PorSeguroCam = new BigDecimal("0.50");
        BigDecimal divisor = new BigDecimal("100");
        BigDecimal MontoRecomendado = nuevoCostoProduccion;
        BigDecimal MontoAseguradoReco = new BigDecimal("0.00");
        BigDecimal CostoMantenimiento = BigDecimal.valueOf(regla.getCostoMantenimiento());

        BigDecimal SumaAseguradaRecomendada = new BigDecimal(0.0);
        ...
        if (regla.getTasa() != 0) {
            if (regla.getCostoProduccion() > 0) {
                SumaAseguradaRecomendada = TotalHec.multiply(new BigDecimal(regla.getCostoProduccion()));
            } else if (regla.getCostoMantenimiento() > 0) {
                SumaAseguradaRecomendada = TotalHec.multiply(new BigDecimal(regla.getCostoMantenimiento()));
            }
        }
        ...
        comparacion = SumaAseguradaRecomendada.compareTo(nuevoMontoAsegura);
        ...
        if (comparacion == 1) {
            txtAnalisisMontoAsegurado.setBackground(Color.YELLOW);
            ObservacionValor = false;
        }
        if (comparacion == -1) {
            txtAnalisisMontoAsegurado.setBackground(Color.RED);
            ObservacionValor = true;
        }


        int diasVigencia = cultivo.getVigenciaDias();

        MontoAseguradoReco = nuevoMontoAsegura;
    ...
    public Regla ObtenerRegla (){
        ...
        //verificamos si es un tipo de cultivo variable

        if(tipoCultivoSelec.getNombre().contains("VARIABLE")){
            txtAnalisisMontoAsegurado.setEnabled(true);
            txtAnalisisMontoAsegurado.setEditable(true);
            txtAnalisisMontoAsegurado.setBackground(Color.green);
        }
        ...
        try {
            ...
            if (StructureFile.exists()) {
                ...
                if (cotizacionActualizable!=null){
                    ...
                    txtDireccionReferencia.setText(cotizacionActualizable.getDireccionSiembra().replace("|A", "Á").replace("|E","É").replace("|I","Í").replace("|O","Ó").replace("|U", "Ú")
                    ...
                    txtNombres.setText(cotizacionActualizable.getNombres().replace("|A", "Á").replace("|E","É").replace("|I","Í").replace("|O","Ó").replace("|U", "Ú")
                            .replace("|a", "á").replace("|e", "é").replace("|i","í").replace("|o","ó").replace("|u","ú")
                            .replace("-", "Ñ").replace("_", "ñ"));
                    txtApellidos.setText(cotizacionActualizable.getApellidos().replace("|A", "Á").replace("|E","É").replace("|I","Í").replace("|O","Ó").replace("|U", "Ú")
                            .replace("|a", "á").replace("|e", "é").replace("|i","í").replace("|o","ó").replace("|u","ú")
                            .replace("-", "Ñ").replace("_", "ñ"));
    ...
    public  String  ValidarCampos(){
        ...
        if (cmb_Parroquia.getSelectedIndex()==0)
            campos = campos+lblParroquia.getText()+"\n*";
        if (cmb_Canton.getSelectedIndex()==0)
